...
# transform categorical age variable to numerical
data <- bind_rows(mf_data, sf_data)
data$Age <- case_when(data$Age == 1 ~ 20,
                      data$Age == 2 ~ 30,
                      data$Age == 3 ~ 40,
                      data$Age == 4 ~ 50,
                      data$Age == 5 ~ 60,
...
tract_sf_loans <- data %>% filter(FamilyType == "sf", BorrowersIncome < 999999999, UPB < 999999999, Age != "NA") %>% 
  group_by(Tract) %>% 
  summarise(sf_num_loans = n(), sf_med_income = median(BorrowersIncome), sf_med_upb = median(UPB), indian = sum(Race_1 == 1), asian = sum(Race_1 == 2), black = sum(Race_1 == 3), hawai = sum(Race_1 == 4), white = sum(Race_1 == 5), mean_age = mean(Age))
...
tract_sf_loans_18_20 <- data %>% filter(year >= 2018, FamilyType == "sf", BorrowersIncome < 999999999, UPB < 999999999, Age != "NA") %>% 
  group_by(Tract) %>% 
  summarise(sf_num_loans = n(), sf_med_income = median(BorrowersIncome), sf_med_upb = median(UPB), indian = sum(Race_1 == 1), asian = sum(Race_1 == 2), black = sum(Race_1 == 3), hawai = sum(Race_1 == 4), white = sum(Race_1 == 5), mean_age = mean(Age))
```
...
# the number of tracts differs between the 2000 census and the 2010 census. For example, in Moon Twp municipality:
# 2000: 4511.01, 4511.02, 4511.03
# 2010: 4511.01, 4511.02, 4511.04, 4511.05, 4513
# The 2000 cunsus contains 416 tracts, though the 2010 census contains 402 tracts

vars2010 <- load_variables(2010, "sf1")
tract_2000 <- get_decennial(geography = "tract", year = 2000, county = "003", state = "PA", variable = c(units = "H001001"), geometry = TRUE)
tract_2010 <- get_decennial(geography = "tract", year = 2010, county = "003", state = "PA", variable = c(units = "H001001"), geometry = TRUE)
